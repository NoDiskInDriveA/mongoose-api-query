module.exports = exports = function apiQueryPlugin(schema) {
    ...
    schema.statics.apiQueryParams = function (rawParams) {
        ...
        var parseSchemaForKey = function (schema, keyPrefix, lcKey, val, operator) {
            ...
            console.log(lcKey);
            console.log(schema.paths[lcKey])
            ...
            console.log(paramType, operator, val)

            if (paramType === "Boolean") {
                if (operator === "exists") {
                    addSearchParam({$exists: val});
                } else {
                ...
                console.log('I AM A DATE', operator, val)
                if (operator === "exists" ||
                    operator === "gt" ||
                    operator === "gte" ||
                    operator === "lt" ||
                    operator === "lte") {
                    newParam = {};
                    newParam["$" + operator] = operator !== 'exists' ? new Date(val) : val;
                    addSearchParam(newParam);
                }
            }
